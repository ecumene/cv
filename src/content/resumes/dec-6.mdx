...
>
    - Delivered features on the Billing and Membership team. Reduced churn and expanded our customer base of 55,000 users by leading customer interviews and implementing feedback.
    - Replaced legacy code with a fresh project. Helped build and deploy a greenfield service that handles thousands of Stripe Transaction Webhooks using Serverless and TypeScript.
    - Used a Citus PostgreSQL database. Implemented Trigram search strategies and indices for more accurate searches and better database performance with a 90% reduction in search time.
    - Wrote Typescript CDK code using Jungle Scout's DevOps platform for deploying new services, monitors, and alarms. Participated in an on-call schedule and resolved high-priority incidents.
</Section>
...
>
    - Reduced waste by importing all existing AWS resources into Terraform states to track resources
    - Laid the foundation for a multi-tenant architecture managed through Terraform while applying SOC2 controls and being mentored by a Senior Software Developer.
    - Setup Multi Availability Zone (AZ) Private Networking (VPC) using AWS NAT Gateway, AWS SSO, S3 Encryption and Tailscale VPN for bullet-proof security with no incidents.
    - Built a backend using Serverless-Stack, AWS S3, Lambda, API Gateway, Cloudwatch and RDS.
    - Utilized AWS Step Functions and Lambda to build a system for processing thousands of PDF pages uploaded in batches. Monitored the pipeline with DataDog Logs, Logs, and APM. 
</Section>
...
>
    - Worked with design to create a design system and a matching component library from scratch with Storybook and private Style Guide (Figma)
...
<div className="text-center my-8">
  All my projects can be found on my GitHub: <a href="https://github.com/ecumene">GitHub Profile</a>
</div>
